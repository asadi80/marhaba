...
const POLICY_META = {
  flexible: { icon: "🟢", labelEn: "Flexible", labelAr: "مرن",   color: "#1D9E75", bg: "#EAF3DE" },
  moderate: { icon: "🟡", labelEn: "Moderate", labelAr: "معتدل", color: "#D97706", bg: "#FEF3C7" },
  strict:   { icon: "🔴", labelEn: "Strict",   labelAr: "صارم",  color: "#e05a5a", bg: "#FCEBEB" },
  custom:   { icon: "✏️", labelEn: "Custom",   labelAr: "مخصص",  color: "#555",    bg: "#f7f6f2" },
};
...
function normalisePolicy(policy) {
  ...
  return {
    ...
    rulePairs: policy.rulePairs ??
      (policy.rules ?? []).map((r) => ({ en: r, ar: r })),
  };
}
...
function CancellationPolicyMini({ policy: rawPolicy, isAr }) {
  ...
  const firstRule = policy.rulePairs.length > 0
    ? (isAr ? policy.rulePairs[0].ar : policy.rulePairs[0].en)
    : null;
...
export default function ListingDetail({ params }) {
  ...
  useEffect(() => {
    ...
    const nights = Math.ceil(
      (createLocalDate(booking.checkOut) - createLocalDate(booking.checkIn)) / 86400000,
    );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res  = await fetch(`/api/listings/${unwrappedParams.id}/view`, { method: "POST" });
        const data = await res.json();
  ...
  const handleBooking = async (e) => {
    e.preventDefault();
    setBookingError(""); setBookingSuccess(""); setIsSubmitting(true);
    if (!booking.checkIn || !booking.checkOut) {
      setBookingError(t.pleaseSelectDates); setIsSubmitting(false); return;
    }
    try {
  ...
  const handleToggleActive = async () => {
    ...
    try {
      const res  = await fetch(`/api/listings/${unwrappedParams.id}`, { method: "PATCH" });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
  ...
  const categoryInfo = listing?.category ? CATEGORIES.find((c) => c.id === listing.category) : null;
  const formatCurrency = (amount) =>
    isAr ? `${amount.toLocaleString()} دينار` : `${amount.toLocaleString()} LYD`;
      ...
      <main className="max-w-[1100px] mx-auto px-6 py-7">
          ...
          <div className="flex flex-wrap items-center gap-3 text-[12px] text-[#888]">
            ...
            <div className="flex items-center gap-1 text-[#666]">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="opacity-70">
                <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
                <circle cx="12" cy="12" r="3" />
              </svg>
              <span>{listingView || listing.view_count || 0} {isAr ? "مشاهدة" : "views"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_360px] gap-5 items-start">
          ...
          <div className="flex flex-col gap-5">
              ...
              <div className="bg-white rounded-2xl border border-black/7 px-5 pb-5 pt-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {listing.amenities.map((a, i) => (
                    <span key={i} className="bg-[#f7f6f2] px-3 py-[5px] rounded-2xl text-[12px] text-[#555]">
                      {a}
                    </span>
                  ))}
                </div>
            ...
            <CancellationPolicyCard policy={listing.cancellation_policy} isAr={isAr} />
            ...
            {!isHost && !isAdmin ? (
              ...
              <div className="bg-white rounded-2xl border border-black/7 border-t-[3px] border-t-[#e8c547] px-5 pb-5">
                ...
                <form onSubmit={handleBooking} className="flex flex-col gap-3.5">
                  ...
                  {nights > 0 && (
                    <div className="bg-[#f7f6f2] rounded-lg px-3 py-3 border border-black/7">
                      <div className="flex justify-between text-[12px] text-[#666] mb-2">
                        <span>
                          {formatCurrency(listing.price)} × {nights} {nights === 1 ? t.night : t.nights}
                        </span>
                  ...
                  >
                    {isSubmitting ? (isAr ? "جاري المعالجة..." : "Processing...") : `${t.bookNow} →`}
                  </button>
                </form>
              </div>
