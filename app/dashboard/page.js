...
const AVATAR_PAL = [{ bg: '#EEEDFE', c: '#3C3489' }, { bg: '#E6F1FB', c: '#0C447C' }, { bg: '#EAF3DE', c: '#27500A' }, { bg: '#FAEEDA', c: '#633806' }, { bg: '#E1F5EE', c: '#085041' }, { bg: '#FBEAF0', c: '#72243E' }];
const avi = name => AVATAR_PAL[(name?.charCodeAt(0) ?? 0) % AVATAR_PAL.length];
const initials = name => name?.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() ?? '';
const statusStyle = s => ({ confirmed: { bg: '#EAF3DE', c: '#27500A' }, pending: { bg: '#FAEEDA', c: '#633806' }, cancelled: { bg: '#FCEBEB', c: '#791F1F' } }[s] ?? { bg: '#F1EFE8', c: '#444' });
...
export default function UserDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (role === 'super_admin' || role === 'admin') { router.push('/admin'); return; }
        if (role === 'host') { router.push('/host-dashboard'); return; }
        setUser(data.user);
        getUserLocation();
        fetchListings();
        fetchBookings();
  ...
  const filterByDistance = radius => {
    ...
    setFiltered(listings.filter(l => l.coordinates && haversine(userLocation.lat, userLocation.lng, l.coordinates.lat, l.coordinates.lng) <= radius));
  ...
  const TABS = [
    ...
    { id: 'bookings', l: `${isAr ? 'الحجوزات' : 'bookings'} (${bookings.length})` }
  ];
        ...
        <nav style={{ background: '#1a1a2e', borderBottom: '1px solid rgba(232,197,71,.15)', position: 'sticky', top: 0, zIndex: 40 }}>
          <div style={{ padding: '0 1rem', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              <Link href="/listings" style={{ fontSize: 11, color: 'rgba(255,255,255,.4)', display: 'none' }} className="sm-show">{isAr ? 'استعرض' : 'browse'}</Link>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: aviBg, color: aviColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 500 }}>{ini}</div>
              <button onClick={handleLogout} style={{ background: 'none', border: '0.5px solid rgba(255,255,255,.15)', borderRadius: 4, color: 'rgba(255,255,255,.4)', fontFamily: 'inherit', fontSize: 11, padding: '4px 10px', cursor: 'pointer' }}>{isAr ? 'خروج' : 'out'}</button>
            </div>
          </div>
        </nav>

        <main style={{ maxWidth: 1200, margin: '0 auto', padding: '1.25rem 1rem' }}>

          {/* Profile strip */}
          <div style={{ background: '#fff', borderRadius: 12, border: '0.5px solid rgba(0,0,0,.07)', padding: '1rem 1.25rem', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
              {[
                ...
                <div key={l} style={{ textAlign: isAr ? 'left' : 'right' }}>
                  <div className="fd" style={{ fontStyle: isAr ? 'normal' : 'italic', fontWeight: 300, fontSize: 22, color: '#111118', lineHeight: 1 }}>{v}</div>
                  <div style={{ fontSize: 10, letterSpacing: '.07em', textTransform: 'uppercase', color: '#bbb', marginTop: 2 }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {activeTab === 'nearby' && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: 10 }}>
                <div className="fd" style={{ fontStyle: isAr ? 'normal' : 'italic', fontWeight: 300, fontSize: 20, color: '#111118' }}>
                  {isAr ? 'الأماكن القريبة' : 'nearby places'}
                  <span style={{ fontSize: 13, fontStyle: 'normal', color: '#999', marginLeft: 8 }}>({filtered.length})</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <label style={{ fontSize: 11, letterSpacing: '.07em', textTransform: 'uppercase', color: '#999' }}>{isAr ? 'نصف القطر' : 'radius'}</label>
                  <select onChange={e => filterByDistance(parseInt(e.target.value))} value={searchRadius} style={{ padding: '7px 10px', background: '#fafaf8', border: '0.5px solid rgba(0,0,0,.12)', borderRadius: 7, fontSize: 12, fontFamily: 'inherit', outline: 'none' }}>
                    {[5, 10, 20, 50, 100].map(v => <option key={v} value={v}>{v} {isAr ? 'كم' : 'km'}</option>)}
                  </select>
                </div>
              </div>
                      ...
                      >
                        <Popup>
                          <div style={{ fontFamily: bodyFont, fontSize: 12, minWidth: 160 }}>
                            ...
                            <div style={{ color: '#888', marginBottom: 6 }}>{l.location}</div>
                            <div style={{ display: 'flex', gap: 6 }}>
                              <Link href={`/listings/${l._id}`} style={{ flex: 1, background: '#1a1a2e', color: '#e8c547', padding: '5px 0', borderRadius: 5, textAlign: 'center', fontSize: 11 }}>{isAr ? 'عرض' : 'view'}</Link>
                              <button onClick={() => getDirections(l)} style={{ flex: 1, background: '#1D9E75', color: '#fff', border: 'none', borderRadius: 5, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>{isAr ? 'اتجاهات' : 'dir'}</button>
                            </div>
                          </div>
                        </Popup>
                      </Marker>
              ...
              {filtered.length > 0 ? (
                <div className="listings-single-column">
                  {filtered.map(l => {
                    const dist = userLocation && l.coordinates ? haversine(userLocation.lat, userLocation.lng, l.coordinates.lat, l.coordinates.lng).toFixed(1) : null;
                    return (
                      <div key={l._id} className="listing-row">
                        <div className="listing-row-image">
                          <img src={l.images?.[0]} alt={l.title} />
                          {dist && <div className="listing-row-distance">{dist} {isAr ? 'كم' : 'km'}</div>}
                        </div>
                        <div className="listing-row-content">
                          <div>
                            <div style={{ fontSize: 16, fontWeight: 500, color: '#111118', marginBottom: 6 }}>{l.title}</div>
                            <div style={{ fontSize: 13, color: '#888', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 4 }}>
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" style={{ color: '#e8c547' }}>
                          ...
                          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                            <Link href={`/listings/${l._id}`} style={{ background: '#1a1a2e', color: '#e8c547', padding: '10px 20px', borderRadius: 8, textAlign: 'center', fontSize: 13, textDecoration: 'none', transition: 'opacity .15s' }}>
                              {isAr ? 'عرض التفاصيل' : 'view details'} →
                            </Link>
                            {l.coordinates && (
                              <button onClick={() => openMaps(l)} style={{ background: '#1D9E75', color: '#fff', border: 'none', borderRadius: 8, padding: '10px 20px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            {l.coordinates && (
                              <button onClick={() => getDirections(l)} style={{ background: '#f5f5f5', color: '#1a1a2e', border: '1px solid #ddd', borderRadius: 8, padding: '10px 20px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div style={{ textAlign: 'center', padding: '4rem 2rem', background: '#fff', borderRadius: 12, border: '0.5px solid rgba(0,0,0,.07)' }}>
                  <div className="fd" style={{ fontStyle: isAr ? 'normal' : 'italic', fontWeight: 300, fontSize: 26, color: '#ccc', marginBottom: '.75rem' }}>{isAr ? 'لا توجد أماكن قريبة' : 'nothing nearby'}</div>
                  <p style={{ fontSize: 14, color: '#999', marginBottom: '1.5rem' }}>{isAr ? `لا توجد أماكن ضمن ${searchRadius} كم` : `No places within ${searchRadius} km`}</p>
                  <button onClick={() => filterByDistance(50)} style={{ background: 'none', border: 'none', color: '#185FA5', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>
          ...
          {activeTab === 'bookings' && (
            <div>
              ...
              {bookings.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: 12, border: '0.5px solid rgba(0,0,0,.07)' }}>
                  <div className="fd" style={{ fontStyle: isAr ? 'normal' : 'italic', fontWeight: 300, fontSize: 26, color: '#ccc', marginBottom: '.75rem' }}>{isAr ? 'لا توجد حجوزات بعد' : 'no bookings yet'}</div>
                  <p style={{ fontSize: 13, color: '#999', marginBottom: '1.5rem' }}>{isAr ? 'اكتشف أماكن رائعة للإقامة' : 'Discover amazing places to stay'}</p>
                  <Link href="/listings" style={{ background: '#1a1a2e', color: '#e8c547', padding: '10px 24px', borderRadius: 8, fontSize: 13 }}>{isAr ? 'استعرض القوائم →' : 'browse listings →'}</Link>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {bookings.map(b => {
                    ...
                    return (
                      <div key={b._id} style={{ background: '#fff', borderRadius: 12, border: '0.5px solid rgba(0,0,0,.07)', padding: '1.1rem 1.25rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 8 }}>
                          <div style={{ fontSize: 14, fontWeight: 500, color: '#111118' }}>{b.listing?.title || (isAr ? 'قائمة' : 'Listing')}</div>
                          <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: '.04em', textTransform: 'uppercase', background: sBg, color: sC, padding: '3px 10px', borderRadius: 20 }}>
                            {b.status === 'confirmed' && (isAr ? 'مؤكد' : 'confirmed')}
                            {b.status === 'pending' && (isAr ? 'قيد الانتظار' : 'pending')}
                            {b.status === 'cancelled' && (isAr ? 'ملغي' : 'cancelled')}
                          </span>
                        </div>
                        <div style={{ fontSize: 12, color: '#888', marginBottom: '1rem' }}>{b.listing?.location}</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(110px,1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
                          {[
                            ...
                            <div key={l}>
                              <div style={{ fontSize: 10, letterSpacing: '.07em', textTransform: 'uppercase', color: '#bbb', marginBottom: 2 }}>{l}</div>
                              <div style={{ fontSize: 12, color: '#111118' }}>{v}</div>
                            </div>
                          ))}
                          <div>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          {b.status !== 'cancelled' && (
                            <>
                              <Link href={`/listings/${b.listing?._id}`} style={{ background: '#1a1a2e', color: '#e8c547', padding: '7px 14px', borderRadius: 7, fontSize: 12 }}>{isAr ? 'عرض القائمة' : 'view listing'}</Link>
                              <button onClick={() => cancelBooking(b._id)} style={{ background: 'none', border: '0.5px solid rgba(163,45,45,.25)', color: '#A32D2D', padding: '7px 14px', borderRadius: 7, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>{isAr ? 'إلغاء' : 'cancel'}</button>
                            </>
                          )}
                          {b.listing?.coordinates && (
                            <button onClick={() => openMaps(b.listing)} style={{ background: '#1D9E75', color: '#fff', border: 'none', borderRadius: 7, padding: '7px 14px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 5 }}>
